...
export class GuiaremisionComponent {
  ...
  obtenerEmpaques(empaque){
    this.api.getEmpaque().subscribe((res:[])=>{
      this.empaques=res;
    })
  ...
  buscarEmpaque(empaque){
  console.log(empaque);
  }
  seleccionarEmpaque(detalle:empaque_detalle){
    var index1 = this.empaquesSeleccionados.findIndex(objeto => objeto.codigo === detalle.codigoProducto);
    if (index1 !== -1) {
      this.empaquesSeleccionados.splice(index1, 1);
    }else{
      var obj={codigo:detalle.codigoProducto.toString(),descripcion:detalle.descripcion,cantidad:detalle.descripcion,unidadmedida:detalle.unidadMedida};
      this.empaquesSeleccionados.push(obj);
    }
    var index2 = this.empaquesSeleccionadosCss.indexOf(detalle.codigoProducto);
    if (index2 !== -1) {
      this.empaquesSeleccionadosCss.splice(index2, 1);
    }else{
      this.empaquesSeleccionadosCss.push(detalle.codigoProducto);
    }
  }
  asignarEmpaque(){
    this.listadoProductoDetalles=this.empaquesSeleccionados;
    this.cerrarModal();
    }
}
